...
func (c *Config) normalize() {
	...
	if c.SQLiteCacheKB <= 0 {
		c.SQLiteCacheKB = 1536
	}
	if c.BusyTimeout <= 0 {
	...
	if c.GraphMaxNodes > 128 {
		c.GraphMaxNodes = 128
	}
	if c.MaxChunkRunes <= 0 {
		c.MaxChunkRunes = 1200
	}
	if c.ChunkOverlap < 0 {
		c.ChunkOverlap = 0
	}
	if c.ChunkOverlap >= c.MaxChunkRunes {
		c.ChunkOverlap = c.MaxChunkRunes / 10
	}
	if c.EmbeddingPollInterval <= 0 {
		c.EmbeddingPollInterval = 5 * time.Second
	}
	if c.EmbeddingMaxAttempts <= 0 {
		c.EmbeddingMaxAttempts = 4
	}
}
